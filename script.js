...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const lastH1Span = h1Spans[h1Spans.length - 1];
    if (lastH1Span && lastH1Span.textContent === 'X') {
        lastH1Span.classList.add('glow');
    }

    // Shuffle animation
    const shuffleElements = document.querySelectorAll('[data-shuffle]');
    shuffleElements.forEach(el => {
        const shuffle = new TypeShuffle(el);
        const shuffleObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    shuffle.trigger('fx1');
                    shuffleObserver.unobserve(el);
                }
            });
        }, {
            threshold: 0.1
        });
        shuffleObserver.observe(el);
    });
